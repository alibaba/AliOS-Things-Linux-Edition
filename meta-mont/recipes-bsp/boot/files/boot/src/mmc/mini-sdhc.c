...
#endif
#ifdef CONFIG_FPGA
#define CONFIG_SYS_SDIO_MAX_CLK 50000000
#define CONFIG_SYS_SDIO_MIN_CLK 400000
#define CONFIG_SYS_SDIO_FPGA_MAX_CLK 25000000
#else
#define CONFIG_SYS_SDIO_MAX_CLK 0
#define CONFIG_SYS_SDIO_MIN_CLK 400000
#endif
...
{
    *(volatile u32 *) (host->ioaddr + (reg & ~(3))) = val;
}

static inline void sdhci_writew(struct sdhci_host *host, u16 val, int reg)
{
    *(volatile u16 *) (host->ioaddr + (reg & ~(1))) = val;
}

static inline void sdhci_writeb(struct sdhci_host *host, u8 val, int reg)
{
    *(volatile u8 *) (host->ioaddr + reg) = val;
}

static inline u32 sdhci_readl(struct sdhci_host *host, int reg)
{
    return *(const volatile u32 *) (host->ioaddr + (reg & ~(3)));
}

static inline u16 sdhci_readw(struct sdhci_host *host, int reg)
{
    return *(const volatile u16 *) (host->ioaddr + (reg & ~(1)));
}

static inline u8 sdhci_readb(struct sdhci_host *host, int reg)
{
    return *(const volatile u8 *) (host->ioaddr + reg);
}
...
{
    {
    "mmc", mmc_cmd, "mmc <start> <blkcnt> <dst> ;mmc read block"}
,};
#endif
#endif
