/*!
*   \file cmd_usb.c
*   \brief USB Descriptor description parser
*   \author
*/
/*
 * (C) Copyright 2001
 ...
 */
#if 0 //def CONFIG_USB_STORAGE
int do_usbboot(int argc, char *argv[])
{
...
int do_usb(int argc, char *argv[])
{
    ...
    if (strncmp(argv[0], "fs", 2) == 0) {
        printf("forcing USB to full speed mode\n");
        usb_force_fs = 1;
        return 0;
    }
    if (strncmp(argv[0], "switch", 6) == 0) {
        if(argc == 2) {
            if((strncmp(argv[1], "0", 1) == 0) ||
                (strncmp(argv[1], "1", 1) == 0))
            {
                usb_select_port = atoi(argv[1]);
                printf("switching USB to port %d\n", usb_select_port);
                return 0;
            }
            else
                printf("only support USB port 0 or 1\n");
        }
        else
            printf("ex: usb switch 0\n");
        return 1;
    }
    if (!usb_started) {
    ...
    if (strcmp(argv[0], "read") == 0) {
        ...
        if (argc == 4) {
            unsigned long addr,blk,cnt,n;
            if (!hextoul(argv[1], &addr) ||
                !hextoul(argv[2], &blk) ||
                !hextoul(argv[3], &cnt) )
            return ERR_HELP;
    ...
    if (strncmp(argv[0], "dev", 3) == 0) {
        if (argc == 2) {
            int dev;
            if (!hextoul(argv[1], &dev))
                return ERR_HELP;
            printf("\nUSB device %d: ", dev);
            stor_dev = usb_stor_get_dev(dev);
            if (stor_dev == NULL) {
...
*/
cmdt cmdt_usb[] __attribute__ ((section("cmdt"))) =
{
    { "usb", do_usb, "usb [reset][tree][info][storage][dev][part][read][write][fs][switch] "} ,
};
...
cmdt cmdt_usb[] __attribute__ ((section("cmdt"))) =
{
    { "usb", do_usb, "usb [reset][tree][info][dev]"} ,
};
#endif
#endif //CONFIG_USB
