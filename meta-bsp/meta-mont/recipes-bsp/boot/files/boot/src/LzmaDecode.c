...
int LzmaDecode(
    ...
    #endif
    unsigned char *outStream, UInt32 outSize,
    UInt32 *outSizeProcessed, int(chk)(void))
{
  UInt32 numProbs = Literal + ((UInt32)LZMA_LIT_SIZE << (lc + lp));
  ...
  *outSizeProcessed = 0;
  while(nowPos < outSize)
  {
    ...
    else             
    {
      ...
      if ((nowPos % 32768) == 0) {
          if (chk && chk())
              return LZMA_RESULT_STOP_BY_USER;
      }
...
int lzmaBuffToBuffDecompress(char *dest, int *destlen, char *src, int srclen, int(chk)(void))
{
    ...
    CBuffer bo;
#endif
#ifdef  _LZMA_FIXED_INT_DATA
    unsigned int _lzmaInternalData[0x4000/4];
#endif


    for (ii = 0; ii < 5; ii++,src++)
    {
        properties[ii]=*src;
    }
    outSize = 0;
    for (ii = 0; ii < 4; ii++,src++)
    {
        outSize += (*(unsigned char*) src) << (ii * 8);
    }
...
#ifdef  _LZMA_FIXED_INT_DATA
    lzmaInternalData = &_lzmaInternalData[0];
#else
...
#ifdef _LZMA_OUT_READ
    {
        ...
        LzmaDecoderInit ((unsigned char *) lzmaInternalData, lzmaInternalSize,
                            lc, lp, pb, dictionary, dictionarySize,
#ifdef _LZMA_IN_CB
        &bo.InCallback
#else
        ...
        for (nowPos = 0; nowPos < outSize;)
        {
            ...
            res = LzmaDecode ((unsigned char *) lzmaInternalData,
                                ((unsigned char *) dest) + nowPos, blockSize,
                                &outSizeProcessed, chk);
            if (res != 0)
            {
                sprintf (rs + strlen (rs), "\nerror = %d\n", res);
                return res;
    ...
    res = LzmaDecode ((unsigned char *) lzmaInternalData, lzmaInternalSize,
                ...
                (unsigned char *) src, compressedSize,
#endif
                (unsigned char *) dest, outSize, &outSizeProcessed, chk);
