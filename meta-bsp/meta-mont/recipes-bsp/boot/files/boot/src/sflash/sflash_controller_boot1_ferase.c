...
#undef CONFIG_PDMA
#undef BAD_BLOCK_MANAGE
#undef SF_ERASE_TO
#define SF_ERASE_TO 1000000
...
#if 0 // irrlevant function, only flash erase API is used
        if (pf_head->type == FLASH_TYPE_NAND)
        {
            if(check_sel == CHECK_BAD_BLOCK)
            {
...
#if 0 // irrlevant function, only flash erase API is used
        if(check_sel == NO_CHECK_BAD_BLOCK)
        {
            /* overwrite boot block number */
            erase_offset = get_boot_block_num();
        }
#endif
        // page managment loop
        inner_addr = addr;
