...
extern int linux_uclibc;
extern int building_for_uclibc;
...
enum opt_code
{
  ...
  OPT_msmartmips,
  OPT_msoft_float,
  OPT_msplit_addresses,
  OPT_msym32,
  OPT_mtune_,
  OPT_muclibc,
  OPT_muninit_const_in_rodata,
  OPT_mvr4130_align,
  OPT_mwarn_framesize_,
  OPT_mxgot,
  OPT_nostdinc,
  OPT_nostdinc__,
  OPT_nostdlib,
...
__pack_f ( fp_number_type * src)
{
      ...
      else
 {
  ...
  dst.value_raw = fraction & ((((fractype)1) << 23) - (fractype)1);
  dst.value_raw |= ((fractype) (exp & ((1 << 8) - 1))) << 23;
  dst.value_raw |= ((fractype) (sign & 1)) << (23 | 8);
  return dst.value;
}
