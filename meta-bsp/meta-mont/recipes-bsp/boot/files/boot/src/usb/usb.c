...
#if 1
// structure must be aligned to 0x20 to optimize performance
static struct devrequest setup_packet __attribute__ ((aligned (0x20)));
#else
...
inline int isprint(unsigned char c)
{
    return ( (c>= 0x20) || (c<'z'));
}
...
int usb_new_device(struct usb_device *dev)
{
    int addr, err;
    int tmp;
#if 0// local variable cann't be aligned
    // structure must be aligned to 0x20 to optimize performance
    unsigned char tmpbuf[USB_BUFSIZ] __attribute__ ((aligned (0x20)));
#else
...
void usb_scan_devices(void)
{
    ...
    for (i = 0; i < USB_MAX_DEVICE; i++) {
        memset((void *) &usb_dev[i], 0, sizeof(struct usb_device));
        usb_dev[i].devnum = -1;
    }
    dev_index = 0;
...
int usb_hub_configure(struct usb_device *dev)
{
    ...
    memset((void *) bitmap, 0xff, (USB_MAXCHILDREN+1+7)/8);
    bitmap = (unsigned char *)&hub->desc.PortPowerCtrlMask[0];
    memset((void *) bitmap, 0xff, (USB_MAXCHILDREN+1+7)/8); /* PowerMask = 1B */
